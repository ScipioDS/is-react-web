      ...
      <div className="relative w-80 h-80">
        <div className="absolute inset-0 flex items-center justify-center">
          <div className="text-center">
            <p className="text-sm text-muted-foreground font-sans mb-1">{t('game.selectWeapon')}</p>
            <p className="text-xs text-muted-foreground/60 font-sans">{t('game.releaseTab')}</p>
          </div>
        </div>
        ...
        <button
          ...
          className={`absolute top-0 left-1/2 -translate-x-1/2 w-24 h-24 rounded-full border-2 transition-all ${
            currentWeapon === 'laser'
        ...
        >
          <Zap className="w-6 h-6 text-game-green" />
          <span className="text-xs font-sans font-semibold">{t('weapons.laser')}</span>
        </button>

        <button
          onClick={() => onWeaponSelect('explosive')}
          onMouseEnter={() => onWeaponSelect('explosive')}
          className={`absolute bottom-8 left-8 w-24 h-24 rounded-full border-2 transition-all ${
            currentWeapon === 'explosive'
              ? 'border-game-red bg-game-red/20 scale-110'
              : 'border-border/50 bg-card/95 hover:border-game-red/50'
          } backdrop-blur flex flex-col items-center justify-center gap-1`}
        >
          <Bomb className="w-6 h-6 text-game-red" />
          <span className="text-xs font-sans font-semibold">{t('weapons.explosive')}</span>
        </button>

        <button
          onClick={() => onWeaponSelect('melee')}
          onMouseEnter={() => onWeaponSelect('melee')}
          className={`absolute bottom-8 right-8 w-24 h-24 rounded-full border-2 transition-all ${
            currentWeapon === 'melee'
              ? 'border-game-yellow bg-game-yellow/20 scale-110'
              : 'border-border/50 bg-card/95 hover:border-game-yellow/50'
          } backdrop-blur flex flex-col items-center justify-center gap-1`}
        >
          <Swords className="w-6 h-6 text-game-yellow" />
          <span className="text-xs font-sans font-semibold">{t('weapons.melee')}</span>
        </button>
      </div>
    </div>
  );
